...
type Store interface {
	// ListWorkflows should return the set of workflows sorted in
	// reverse-chronological order (newest first) of the last time they were run.
	// When two LastRun times are equal, Workflows should alpha sort the names
	...
	// GetWorkflowByName gets a workflow with the corresponding name field.
	// usually matches the dataset name
	GetWorkflowByName(ctx context.Context, name string) (*Workflow, error)
	// Workflow gets a workflow by it's identifier
	GetWorkflow(ctx context.Context, id string) (*Workflow, error)
	// PutWorkflow places a workflow in the store. Putting a workflow who's name
	// already exists must overwrite the previous workflow, making all workflow
	// names unique
...
// GetWorkflowByName gets a workflow with the corresponding name field. usually
...
// PutWorkflow places a workflow in the store. If the workflow name matches the
// name of a workflow that already exists, it will be overwritten with the new
// workflow
