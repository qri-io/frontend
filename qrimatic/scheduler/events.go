...
const (
	// ETWorkflowScheduled fires when a workflow is registered for updating,
	// or when a scheduled workflow changes
	// payload is a Workflow
	// subscriptions do not block the publisher
	ETWorkflowScheduled = event.Type("cron:WorkflowScheduled")
	// ETWorkflowUnscheduled fires when a workflow is removed from the update schedule
	// payload is a Workflow
	// subscriptions do not block the publisher
